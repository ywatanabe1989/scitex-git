...
class TestCreateChildGit:
    ...
    def test_create_child_git_already_exists(self):
        """When repo already exists, returns project_dir without re-init.

        scitex.writer.verify_tree_structure is optional in standalone
        scitex-git: if it isn't installed (or it fails), the function
        still returns project_dir.
        """
        with tempfile.TemporaryDirectory() as temp_dir:
            project_dir = Path(temp_dir)
